...
- 活跃运行分支：`codex/pyside6-migration`；已提交 checkpoint 为 `ed66411`；PySide6 环境固定为 `.venv-pyside`。
- 当前桌面启动器指向该分支的工程；`AnoTime-macOS` 不属于本项目维护范围。
- 完整测试基线：已提交 checkpoint 的验证为 486 项通过；当前未提交玻璃字幕平滑批次为 489 项通过，尚待 macOS 实机验收与独立提交。只有在变更后重新运行，才能更新该数字。
- Apple ASR 是默认延迟与资源基线；Parakeet 是实验路径，不得自动设为默认。
- 当前第一待办：验收未提交的玻璃字幕平滑批次；不得重新启用已失败的固定当前句/历史隔离布局。之后再进行 Phase 3 的受控 EOU 回放实验与 Phase 4 的 macOS 实机验收；不能只靠调低切段阈值。
